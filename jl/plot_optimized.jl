...
function plot_optimized(ax,
    ...
    for d in unique(df.d)
        ...
        #for rep in 1:1
            ...
            lines!(ax, df_herehere.time, df_herehere.mean_P, alpha=0.2, color=d, colorrange=(0.1, 100.0), colormap=cgrad(Asteroidea.raquelcol, [0.1, 1.0, 100.0] ./ 100.0, categorical = true), label=string(round(d; digits=1)))
        end
    end
end
...
# Population fits
fig, ax_Afri, ax_APS = prep_popplot()

for rep in 1:nrep

df_Afri = CSV.read("../output/sweep_Afrikaans_abc_$rep.csv", DataFrame)
df_APS = CSV.read("../output/sweep_APS_abc_$rep.csv", DataFrame)

#subset!(df_Afri, [:d, :aspectratio, :seed] => ((a,b,c) -> a .== 1.0 .&& 1.0 .== b))
#subset!(df_APS, [:d, :aspectratio, :seed] => ((a,b,c) -> a .== 1.0 .&& 1.0 .== b))
...
plot_population(ax_Afri, df_Afri, data_afrikaans, offset_afrikaans; maxtime = 1850)
plot_population(ax_APS, df_APS, data_aps, offset_aps; maxtime = 1650)

#poly!(ax_Afri, Point2f[(Afri_L2_start, -10000), (Afri_L2_start, 80_000), (Afri_L2_end, 80_000), (Afri_L2_end, -10000)], alpha=0.1, color=:orange)
#poly!(ax_APS, Point2f[(APS_L2_start, -10000), (APS_L2_start, 80_000), (APS_L2_end, 80_000), (APS_L2_end, -10000)], alpha=0.1, color=:orange)
end

axislegend(ax_Afri, "learner type", position = (0.02, 0.95), merge=true)

save("../plots/fitted-demographics.pdf", fig)



# ABC fits
fig, ax_Afri, ax_APS = prep_optiplot()
...
save("../plots/optimized-simulations.pdf", fig)
